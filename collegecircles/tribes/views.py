...
from collegecircles.tribes.forms import TribeForm, TribeUpdateForm
...
@login_required
def create(request, form_class=TribeForm, template_name="tribes/create-t.html"):
...
def tribes(request, template_name="tribes/tribes-t.html"):
...
@login_required
def your_tribes(request, template_name="tribes/your_tribes-t.html"):
    return render_to_response(template_name, {
...
from collegecircles.swaps.models import Offer
# helper for tribe page
def offers_for_tribe(tribe, limit=-1):
    all_offers = Offer.objects.filter(circles=tribe)
    # print all_offers
    if limit==-1:
        return all_offers
    else:
...
def tribe(request, group_slug=None, form_class=TribeUpdateForm,
    ...
    recent_offers = offers_for_tribe(tribe, 10)
